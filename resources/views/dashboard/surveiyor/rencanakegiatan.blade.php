            ...
            @foreach ($rencanakegiatan as $rk)
              <tbody>
                <tr>
                  ...
                  <td>{{ $rk->personal }}</td>
                  {{-- <td>
                    <a href="{{ route('surveiyor.rencanakegiatan.detail', $rk->id_rencana) }}" class="btn btn-sm btn-outline-success mr-1 mb-1">
                      View
                    </a>
                  </td> --}}
                </tr>
    ...
    <div class="modal-dialog modal-lg modal-dialog-scrollable" role="document">
      <div class="modal-content">
        ...
        <div class="modal-body">
          <form action="rencanakegiatansyor/create" method="POST" enctype="multipart/form-data">
            {{ csrf_field() }}
            <div class="form-row">
              <div class="name">Bulan - Tahun</div>
              <div class="value">
                <input required class="form-control bg-white border-1 small mr-4 input-month" type="text" name="bulan_tahun">
              </div>
            </div>
            ...
            <div class="form-row">
              ...
              <div class="value">
                <select required class="form-control bg-white border-1 small mr-4" placeholder="Pilih Data" aria-label="select" name="personal">
                  <option value="">-Pilih-</option>
  ...
  <div class="modal fade" id="editrk" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg modal-dialog-scrollable" role="document">
      <div class="modal-content">
        ...
        <div class="modal-body">
          <form action="rencanakegiatansurveiyor/update" method="POST" enctype="multipart/form-data">
            {{ csrf_field() }}
            ...
            <div class="form-row">
              <div class="name">Jenis Kegiatan</div>
              <div class="value">
                <select required id="edit-rk-jenis" class="form-control bg-white border-1 small mr-4" placeholder="Pilih Data" aria-label="select" name="jeniskegiatan">
                  <option value="">-Pilih-</option>
                  @foreach ($jeniskegiatan as $jenis)
                    <option value="{{ $jenis->namakegiatan }}"> {{ $jenis->namakegiatan }}</option>
                  @endforeach
            ...
            <div class="form-row">
              <div class="name">Personal</div>
              <div class="value">
                <select required id="edit-rk-personal" class="form-control bg-white border-1 small mr-4" placeholder="Pilih Data" aria-label="select" name="personal">
                  <option value="">-Pilih-</option>
                  @foreach ($users as $user)
                    <option value="{{ $user->nama_pengguna }}"> {{ $user->nama_pengguna }}</option>
                  @endforeach
